...
\Abstract{palavras-chave}{6G, Federação de Rede, Computação de Borda Multi-Acesso, Fatias de Rede, Redes Públicas/Privadas}
         {resumo}{\selectlanguage{portuguese}As redes 6G do futuro terão de integrar de forma transparente ambitentes altamente hetergéneos, combinando infraestruturas de nuvem centralizadas com computação de borda. Esta proposta de tese aborda os desafios da federação de redes multidomínio, focando-se na orquestração dinâmica de fatias de rede e na computação de borda multi-acesso (MEC) em domínios públicos e privados. Ao explorar cenários altamente exigentes - especificamente, estádios inteligentes de alta dencidade e operações entre fronteiras de Proteção Pública e Socorro em Catástrofes (PPDR) - esta tese de doutoramento visa desenvolver mecanismos de federação seguros, escaláveis e sem estado. O trabalho irá propor uma arquitetura inovadora para a orquestração entre domínios, tirando partido das interfaces da Ligação Este-Oeste (EWBI) e de interligação programável para garantir a prestação contínua de serviços durante cargas de rede imprevisíveis e respostas a emergências críticas.}
